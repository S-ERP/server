...
public class Alvaro {
            ...
            while ((linea = reader.readLine()) != null) {
                ...
                if (!linea.isEmpty()) {
                    ...
                    if (partes.length == 2) {
                        ...
                        String nombreArchivo = dateAndFile.length > 1 ? dateAndFile[1].trim() : "";
                        String ruta = remoteDir + "/" + nombreArchivo;

                        if (nombreArchivo.isEmpty()) {
                            System.out.println("[ALVARO] Advertencia: Nombre vacío en línea: " + linea);
                            continue;
                        }

                        JSONObject backupInfo = new JSONObject();
                        backupInfo.put("nombre", nombreArchivo);
                        ...
                        System.out.println("[ALVARO] Backup agregado - Nombre: " + nombreArchivo + ", Ruta: " + ruta);
                    }
                }
            }
    ...
    public static void verBackupFrontend(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            String comando = "ssh \"" + sshHost + "\" \"ls -lhd " + remoteDir + "/build* 2>/dev/null | awk '{print \\$6, \\$7, \\$8, \\$9, \\\"(\\\" \\$5 \\\")\\\"}' \"";
            System.out.println("[ALVARO] Ejecutando: " + comando);
            ...
            while ((linea = reader.readLine()) != null) {
                ...
                if (!linea.isEmpty()) {
                    ...
                    if (partes.length == 2) {
                        ...
                        String fecha = dateAndFile[0].trim();
                        String nombreCarpeta = dateAndFile.length > 1 ? dateAndFile[1].trim() : "";
                        String ruta = remoteDir + "/" + nombreCarpeta;
                        ...
                        JSONObject backupInfo = new JSONObject();
                        backupInfo.put("nombre", nombreCarpeta);
                        backupInfo.put("ruta", ruta);
                        backupInfo.put("tamaño", tamaño);
                        ...
                        backupsArray.put(backupInfo);
                        System.out.println("[ALVARO] Backup agregado - Nombre: " + nombreCarpeta + ", Ruta: " + ruta);
                    }
                }
            }
    ...
    public static void eliminarBackupBackend(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            System.out.println("[ALVARO] Ruta a eliminar: " + rutaBackup);
            System.out.println("[ALVARO] Objeto recibido: " + obj.toString());

            if (rutaBackup.isEmpty()) {
                obj.put("estado", "error");
                obj.put("error", "Ruta del backup no especificada");
                System.out.println("[ALVARO] Error: Ruta vacía");
                return;
            }
            ...
            String linea;
            while ((linea = reader.readLine()) != null) {
                System.out.println("[ALVARO] Output: " + linea);
            }
            ...
            obj.put("estado", "error");
            obj.put("error", "Excepción: " + e.getMessage());
    ...
    public static void restaurarBackupFrontend(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            String sshHost = "servisofts@192.168.2.5";
            String rutaBackup = obj.optString("ruta", "");
            String buildDir = "/home/servisofts/servicios/serp/entornos/serp/build";

            System.out.println("[ALVARO] Ruta del backup a restaurar: " + rutaBackup);
            System.out.println("[ALVARO] Ruta destino (build): " + buildDir);
